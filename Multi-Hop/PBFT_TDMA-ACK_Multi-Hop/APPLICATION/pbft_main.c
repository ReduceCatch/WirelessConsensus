...
void pbft(){
	...
	//2-29 : data_body  （ACK 则2表示是哪个类型的数据1->propose 2->choice 3->commit）//多轮共识这里需要修改
...
/*****************************Global_Consensus**************************************************************************************************************************************************************************/	
	...
	/*****************************PREPREPARE**********************************/
	if(id == Global_Consensus_Leader) Global_New_Proposal();
...
//		if(id==Global_Consensus_Leader+1)delay_ms(300);
		Global_Send_PREPARE(my_choice);
		prepare_flag = 1;
...
void Global_Send_ACK(){
	...
	for(u8 m=1;m<=N;m++)Send_Data_buff[3+m+N] = Global_Commit[m];
